...
class SQLBuilder_fromstructurednotesxxxxx(   ):
    ...
    def __init__( self, table_info  ):
        """
        what it says .. for now it seems sub classes should make super call
        """
        ...
        # arguments push in ... or usual args ??
    ...
    def reset( self,  ):
        ...
        self.select_name        = "A select- name not set "   # user oriented name, same as in drop down -- can drop down pass itself with lambda
   ...
   # ----------------------------------------------
    def grouping_row_function( self  ):
        ...
        or rather than do something new just use a pseudo column ... but how to init it
  ...
  # ----------------------------------------------
    def go( self,  ):
        ...
        arg:  which select in builder to run, later when subclass just go
        """
        print( "SQLBuilder builder.go() ")
        AppGlobal.gui.clear_message_area( ) # check this actually works in real time !!
        try:
            self.this_select()          # configured for one of self.  ... with subleasing always the same
